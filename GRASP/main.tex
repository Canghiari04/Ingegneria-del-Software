...
\textit{Soluzione}\\
La soluzione consiste nella sistematizzazione della classe software che acquisisca la responsabilità. L'espressione riportata indica che l'assegnamento di responsability dovrebbe accadere nei confronti di classi già esistenti, scegliendo tra quelle che comprendano il numero maggiori di informazioni necessarie.\vspace*{14pt}\\
\textit{Caso di studio}\\
In relazione alla costruzione di un sistema software in grado di attribuire la spesa finale che il cliente dovrà sostenere pur di ottenere i prodotti selezionati, occorre definire il responsabile che definirà il costo totale. Secondo il pattern presentato, innanzitutto si osserva la classe software che contenga il numero maggiore di informazioni richieste, analizzando sia il design model che il domain model, nonostante occorre dare principale rilevanza al primo citato.\vspace*{14pt}\\Tuttavia assumendo la totale mancanza di un design model di riferimento, l'unico diagramma in grado di illustare ciò che sia necessario per un'implementazione software consiste in un domain model. Per cui, data la presenza della classe \textit{Sale} sarà traslata anche all'interno del design model, a cui sarà imposta la responsabilità di conoscere il totale della spesa. Nonostante questo approccio supporta un livello rappresentazionale piuttosto basso, il quale si sofferma concretamento su ciò che deriva da un domain model legato strettamente alla realtà.\vspace*{14pt}\\Per cui è necessario conoscere l'intera \textit{lista dei prodotti} voluti e le \textit{subtotals} di ogni istanza dedicata. In questo contesto l'uso di un \textit{diagramma di sequenza} potrebbe aiutare nell'intento. Dalla figura sottostante, si nota come i due diagrammi possano variare di molto a livello di responsabilità, dove non solo la classe \textit{Sale} rappresenterà un ruolo fondamentale. Infatti si osserva la presenza di due classi software aggiuntive, quali \textit{SaleLineItems} e \textit{ProductSpecification}, dove la prima provvederà a restituire il \textit{subtotal} dei prodotti scannerizzati fino ad un dato momento, mentre la corrisposta provvede a indicare il prezzo specifico dell'articolo. 
\begin{center}
...
\end{itemize}\vspace*{7pt}
\textit{Soluzione}\\
In pratrica, il \textit{livello di accoppiamento} non può essere considerato come un'entità a sè stante da ulteriori principi, tra cui \textit{Information Expert} oppure \textit{High Coesion}. Spesso nelle proposte valutative è considerato come un fattore attuabile solamente con la sequenzialità implementativa della progettazione.\vspace*{14pt}\\
\textit{Caso di studio}\\
Si considerino le classi \textit{Register}, \textit{Payment} e \textit{Sale}. Assumendo che l'obiettivo ritragga la creazione un'istanza di pagamento affinchè sia associata alla classe \textit{Sale}. In una raffigurazione fedele alla realtà, il pattern \textit{Creator} suggerisce che sia \textit{Register} l'artefice della creazione di istanze di \textit{Payment}, il quale potrebbe successivamente inoltrare tutto il necessario a \textit{Sale}. In questo contesto, l'assegnamento di una duplice responsabilità a \textit{Register}, prevede che la classe in questione abbia a disposizione tutte le informazioni necessarie, pur di riuscire nell'intento.\vspace*{4pt}
\begin{center}
